...
export class Plates {
  ...
  constructor(
    ...
    this.maxLength = Math.max(
      8,
      Math.round(options.lengthScale * Math.sqrt(this.graph.junctionCount)),
    );
  ...
  addSeed(junction: number): void {
    ...
    const seed = readVec3(this.graph.junctionPos, junction);
    const u = launches[0].dir;
    const v = normalise(cross(seed, u));
  ...
  private tangentToward(from: number, to: number): Vec3 {
    const fromPos = readVec3(this.graph.junctionPos, from);
    const toPos = readVec3(this.graph.junctionPos, to);
    return normalise(tangent(toPos, fromPos));
  }
   ...
   */
  private forwardHeading(from: number, to: number): Vec3 {
    const fromPos = readVec3(this.graph.junctionPos, from);
    const toPos = readVec3(this.graph.junctionPos, to);
    return normalise(sub(scale(toPos, dot(fromPos, toPos)), fromPos));
  }
}
