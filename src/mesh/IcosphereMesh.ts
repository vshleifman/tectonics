...
import type { BoundaryGraph, CellMesh } from "./CellMesh";

/** The 12 base icosahedron vertices (golden-ratio construction, unit sphere). */
const buildBaseIcosahedron = (): { verts: Vec3[]; faces: Vec3[] } => {
...
export class IcosphereMesh implements CellMesh {
  ...
  constructor(level: number) {
    ...
    for (let i = 0; i < verts.length; i++) {
      writeVec3(this.centres, i, verts[i]);
    }
    ...
    const faceCentroids: Vec3[] = faces.map((face) => {
    ...
    for (let f = 0; f < faces.length; f++) {
      writeVec3(junctionPos, f, faceCentroids[f]);
    }
    ...
    this.neighbourList = new Int32Array(this.neighbourOffsets[verts.length]);
    this.neighbourArcList = new Int32Array(this.neighbourOffsets[verts.length]);
    for (let i = 0; i < verts.length; i++) {
    ...
    for (let i = 0; i < verts.length; i++) {
      this.polyOffsets[i + 1] = this.polyOffsets[i] + incidentFaces[i].length;
    }
  ...
  position(i: number): Vec3 {
    return readVec3(this.centres, i);
  }
  ...
  polygon(i: number): readonly Vec3[] {
    ...
    const out: Vec3[] = [];
    for (let k = start; k < end; k++) out.push(readVec3(this.polyCorners, k));
    return out;
  }
...
const subdivideOnce = (
  verts: Vec3[],
  faces: Vec3[],
): { verts: Vec3[]; faces: Vec3[] } => {
  const newVerts: Vec3[] = verts.slice();
...
const orderCornersAround = (centre: Vec3, corners: Vec3[]): Vec3[] => {
...
const sphericalTriangleArea = (a: Vec3, b: Vec3, c: Vec3): number => {
...
const sphericalPolygonArea = (centre: Vec3, corners: Vec3[]): number => {
  let total = 0;
